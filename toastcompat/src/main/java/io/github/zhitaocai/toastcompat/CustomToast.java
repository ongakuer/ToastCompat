...
public class CustomToast implements IToast {
     ...
     * 维护toast的队列
     */
    private static BlockingQueue<CustomToast> mQueue = new LinkedBlockingQueue<>();
     ...
     */
    @TargetApi(Build.VERSION_CODES.JELLY_BEAN_MR1) @Override public IToast setGravity(int gravity,
    ...
    private static void activeQueue() {
        CustomToast customToast = mQueue.peek();
        if (customToast == null) {
            ...
            mHandler.post(customToast.mShow);
            mHandler.postDelayed(customToast.mHide, customToast.mDurationMillis);
            mHandler.postDelayed(mActive, customToast.mDurationMillis);
        }
    }
    ...
    private final Runnable mShow = new Runnable() {
        @Override public void run() {
            try {
                // 在 MIUI 8 开发版上禁止通知, 会禁止 WindowManager.LayoutParams.TYPE_TOAST 权限,导致崩溃。。。
                // 考虑到其他系统也可能这样修改, Try 一下防止崩溃
                handleShow();
            } catch (Exception e) {
            }
        }
